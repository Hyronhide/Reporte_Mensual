...
def reportes_view(request, id_mes):
	...
	if request.method == "POST":		
		...
		if form.is_valid():
			...
			msg2.attach('%s.xlsx'%(user.first_name),fd2.read(), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
			fd3 = open("%s"%(file_path),"rb")
			msg3.attach('%s.xlsx'%(user.first_name),fd3.read(), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
